 ...
 * A 股上市银行高股息数据集。
 *
 * 数据来源：各银行年度报告、东方财富、同花顺等公开渠道。
 * 每股分红(dps)与现价(priceSeed)均为「兜底种子值」：
 *   - dps 默认由 server function 从东方财富分红接口实时算出(最近已实施完整年度合计)，
 *     抓取失败时才回退到此处的种子值；
 *   - priceSeed 同理，实时行情抓取失败时才使用。
 * 市净率(pb)为参考值。股息率 = 每股分红 / 现价 × 100%，会随实时股价动态变化。
 *
 * ⚠️ 本数据仅供学习与研究，数值为参考值，可能与最新公告存在偏差，不构成任何投资建议。
 */

export const BANK_DATA_DATE = '2026-06-17'

export type MarketCode = 'SH' | 'SZ'

export type BankCategory = '国有大行' | '股份行' | '城商行' | '农商行'
...
export const BANKS: Array<BankSeed> = [
  // 国有大行
  ...
  // 股份制银行
  { code: '600036', name: '招商银行', market: 'SH', category: '股份行', dps: 2.0, pb: 1.05, priceSeed: 44.0 },
  { code: '601166', name: '兴业银行', market: 'SH', category: '股份行', dps: 1.04, pb: 0.58, priceSeed: 21.0 },
  { code: '600000', name: '浦发银行', market: 'SH', category: '股份行', dps: 0.32, pb: 0.55, priceSeed: 11.0 },
  ...
  { code: '002142', name: '宁波银行', market: 'SZ', category: '城商行', dps: 0.9, pb: 0.9, priceSeed: 27.0 },
  ...
  { code: '601997', name: '贵阳银行', market: 'SH', category: '城商行', dps: 0.47, pb: 0.42, priceSeed: 6.0 },
...
/** 按股息率绝对水平给银行做高股息分级。 */
export function rateBankYield(dividendYield: number): DividendRating {
  if (dividendYield >= 6) {
