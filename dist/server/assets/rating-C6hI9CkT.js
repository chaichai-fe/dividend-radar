import { S as Subscribable, v as pendingThenable, w as resolveQueryBoolean, s as shallowEqualObjects, x as resolveStaleTime, a as noop, y as environmentManager, z as isValidTimeout, A as timeUntilStale, C as timeoutManager, D as focusManager, E as fetchState, F as replaceData, n as notifyManager, b as shouldThrowError, u as useQueryClient } from "./router-aMmKAEBC.js";
import { r as reactExports } from "./worker-entry-sq6vnRaQ.js";
...
function valuate(yieldNow, yieldLow, yieldHigh) {
  const span = Math.max(yieldHigh - yieldLow, 1e-4);
...
const FACTOR_WEIGHTS = { yield: 0.5, pb: 0.3, pe: 0.2 };
function pctHigherCheaper(now, low, high) {
  const span = Math.max(high - low, 1e-4);
  return Math.min(100, Math.max(0, (now - low) / span * 100));
}
function pctLowerCheaper(now, low, high) {
  const span = Math.max(high - low, 1e-4);
  return Math.min(100, Math.max(0, (high - now) / span * 100));
}
function valuateMulti(input) {
  const factors = {
  ...
  const composite = factors.yield * FACTOR_WEIGHTS.yield + factors.pb * FACTOR_WEIGHTS.pb + factors.pe * FACTOR_WEIGHTS.pe;
...
const BOND_YIELD_10Y = 1.78;
function equityBondSpread(dividendYield) {
  return Number((dividendYield - BOND_YIELD_10Y).toFixed(2));
}
export {
  BOND_YIELD_10Y as B,
  equityBondSpread as e,
  useQuery as u,
  valuateMulti as v
};
