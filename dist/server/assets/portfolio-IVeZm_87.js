...
const snapshots = sqliteTable("snapshots", {
  ...
  takenAt: integer("taken_at", { mode: "timestamp" }).default(sql`(unixepoch())`).notNull(),
...
const schema = { holdings, snapshots };
function getDb() {
  return drizzle(env.DB, { schema });
}
const listHoldings_createServerFn_handler = createServerRpc({
  id: "093a08c04a040019d99379d441a551d685398636abb70dc186413077309aba3f",
  name: "listHoldings",
  filename: "src/server/portfolio.ts"
}, (opts) => listHoldings.__executeServer(opts));
const listHoldings = createServerFn({
  method: "GET"
}).handler(listHoldings_createServerFn_handler, async () => {
  const db = getDb();
  const rows = await db.select({
    code: holdings.code,
    amount: holdings.amount
  }).from(holdings).orderBy(asc(holdings.createdAt));
  return rows;
});
const addHolding_createServerFn_handler = createServerRpc({
  id: "423814c46ccea2f96f45773aa5c86386e3412003bc039b59903b1ded3582e370",
  name: "addHolding",
  filename: "src/server/portfolio.ts"
}, (opts) => addHolding.__executeServer(opts));
const addHolding = createServerFn({
  method: "POST"
}).validator((data) => data).handler(addHolding_createServerFn_handler, async ({
  data
}) => {
  const db = getDb();
...
const setHoldingAmount_createServerFn_handler = createServerRpc({
  id: "5e22afacbfa95f0c269e78516aad11be99d61a2e781c02a2cf9d44982ebdb8e4",
  name: "setHoldingAmount",
  filename: "src/server/portfolio.ts"
}, (opts) => setHoldingAmount.__executeServer(opts));
const setHoldingAmount = createServerFn({
  method: "POST"
}).validator((data) => data).handler(setHoldingAmount_createServerFn_handler, async ({
  data
}) => {
  const db = getDb();
  await db.insert(holdings).values({
...
const removeHolding_createServerFn_handler = createServerRpc({
  id: "bad3290d423157d7ddcd5256ee9b0c9c99dea0745eb675e7cca360f4b47396ed",
  name: "removeHolding",
  filename: "src/server/portfolio.ts"
}, (opts) => removeHolding.__executeServer(opts));
const removeHolding = createServerFn({
  method: "POST"
}).validator((data) => data).handler(removeHolding_createServerFn_handler, async ({
  data
}) => {
  const db = getDb();
...
export {
  addHolding_createServerFn_handler,
  listHoldings_createServerFn_handler,
  removeHolding_createServerFn_handler,
  setHoldingAmount_createServerFn_handler
};
